...
import { readStoredTeam, SELECTED_TEAM_CACHE_KEY, type StoredTeam } from "@/infrastructure/identity/storedTeam";
...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        const res = await fetch(`/api/inbox/unread-count?teamId=${encodeURIComponent(stored.rosterId)}`);
        if (res.ok && !cancelled) {
