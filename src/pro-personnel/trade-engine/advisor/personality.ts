...
//
// Keys are immutable identity base slugs, never editable display names.
...
export function getPersonality(baseSlug: string | null | undefined): TeamPersonality {
  if (!baseSlug) return DEFAULT_PERSONALITY;
  return TEAM_PERSONALITIES[baseSlug.toLowerCase()] ?? DEFAULT_PERSONALITY;
}
