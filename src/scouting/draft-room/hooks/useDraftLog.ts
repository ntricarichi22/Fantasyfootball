 ...
 * `/api/scouting/draft/log` (which already filters to `is_announced = true` by default).
 */
...
/** One slot in the upcoming draft order (from `/api/scouting/draft/order`). */
 ...
 *
 * - `/api/scouting/draft/order` returns one row per slot (round 1 of teamCount picks),
 *   resolved through `pickOwnerByPickKey` so traded picks show the new owner.
 * - `/api/scouting/draft/log` returns rows where `is_announced = true`, ordered by
 *   `pick_index` asc.
...
export function useDraftTicker({ disabled = false }: { disabled?: boolean } = {}): UseDraftTickerResult {
  ...
  const fetchOrder = useCallback(async () => {
    try {
      const res = await fetch("/api/scouting/draft/order", { cache: "no-store" });
      if (!res.ok) return;
      const json = await res.json();
      const rows: RawRow[] = Array.isArray(json?.data) ? json.data : [];
      const next = rows
