...
export default function RosterModal({
  ...
  const filtered = searchLower
    ? assets.filter((a) => a.name.toLowerCase().includes(searchLower) || a.meta.toLowerCase().includes(searchLower))
    : assets;

  const tiers: { key: RosterAsset["tier"]; label: string; showAI?: boolean }[] = [
    { key: "priority", label: "Priority targets", showAI: true },
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div style={{ fontFamily: FH, fontWeight: 800, fontSize: 15, color: "#FEFCF9" }}>
            {teamName}
          </div>
          <div
            onClick={onClose}
            style={{ fontSize: 18, color: "#8C7E6A", cursor: "pointer", fontWeight: 700 }}
          >
            ✕
          </div>
        </div>
        ...
        >
          <input
            type="text"
            placeholder={`Search ${teamName} roster…`}
            value={search}
