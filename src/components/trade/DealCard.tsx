...
export default function DealCard({ myTeamId, teams, assets, onRemove, onAddClick, threeTeam }: Props) {
  if (threeTeam) {
    const teamDeals: TeamDeal[] = teams.map((t) => ({
      teamId: t.id,
      teamName: t.name,
    ...
    return (
      <div style={{ background: "#185FA5", border: "2.5px solid #1A1A1A", boxShadow: "4px 4px 0 #1A1A1A" }}>
        {teamDeals.map((td, i) => (
          <div key={td.teamId} style={{ padding: "12px 14px", borderBottom: i < teamDeals.length - 1 ? "1.5px solid rgba(255,255,255,0.15)" : "none" }}>
            <div style={{ fontFamily: FH, fontWeight: 800, fontSize: 12, color: "#FEFCF9", marginBottom: 8 }}>{td.teamName}</div>
        ...
        <div onClick={onAddClick} style={{ borderTop: "1.5px solid rgba(255,255,255,0.15)", padding: "8px 14px", textAlign: "center", fontSize: 10, color: "rgba(255,255,255,0.4)", cursor: "pointer", fontFamily: F }}>+ Add</div>
      </div>
    );
  }
    ...
    <div style={{ background: "#185FA5", border: "2.5px solid #1A1A1A", boxShadow: "4px 4px 0 #1A1A1A" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
        <div style={{ padding: "14px 16px", borderRight: "1.5px solid rgba(255,255,255,0.15)" }}>
          ...
          {mySends.length === 0 && <div style={{ border: "1.5px dashed rgba(255,255,255,0.25)", padding: "7px", textAlign: "center", fontSize: 10, color: "rgba(255,255,255,0.4)" }}>+ Add from your roster</div>}
        </div>
        <div style={{ padding: "14px 16px" }}>
          ...
          {myReceives.length === 0 && <div style={{ border: "1.5px dashed rgba(255,255,255,0.25)", padding: "7px", textAlign: "center", fontSize: 10, color: "rgba(255,255,255,0.4)" }}>+ Add from their roster</div>}
        </div>
      </div>
      <div onClick={onAddClick} style={{ borderTop: "1.5px solid rgba(255,255,255,0.15)", padding: "8px 14px", textAlign: "center", fontSize: 10, color: "rgba(255,255,255,0.4)", cursor: "pointer", fontFamily: F }}>+ Add</div>
    </div>
