...
mapfile -t pending < <(find "$root/supabase/migrations" -maxdepth 1 -type f \
  \( -name '012_*.sql' -o -name '013_*.sql' -o -name '014_*.sql' -o -name '015_*.sql' -o -name '016_*.sql' -o -name '017_*.sql' -o -name '018_*.sql' -o -name '019_*.sql' \) -print | sort)
[[ ${#pending[@]} == 8 ]] || { echo "Expected exactly migrations 012-019 for staged rollout; found ${#pending[@]}." >&2; exit 1; }
...
restore_pending
for version in 012 013 014 015 016 017 018 019; do
