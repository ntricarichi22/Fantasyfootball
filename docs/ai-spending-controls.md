...
The checked-in Sonnet 5 example uses USD micros per million tokens: `2000000`, `10000000`, `2500000`, `4000000`, and `200000`. Other models, including the currently used `claude-sonnet-4-5`, remain fail-closed unless every required price is privately configured from a current official source. Prompt caching and paid server tools are rejected by the request envelope. Sonnet 5 requests reject custom `temperature`, `top_p`, and `top_k`, avoiding their incompatibility with default adaptive thinking.
