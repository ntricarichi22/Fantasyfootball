...
Production backup coverage, plan, retention, point-in-time recovery (PITR), last
successful backup, and restorable dates are **not verified**. This checkout has
no Supabase CLI project link, Management API credential, database connection
string, or dashboard access. No production query or restore was performed.
