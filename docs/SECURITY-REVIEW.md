...
Authorized read-only catalog metadata supplied on 2026-09-12 confirmed critically broad live Data API grants and missing/open RLS. See `SECURITY-INTEGRATION-REPORT.md` for provenance and exact disposition. No production setting or policy was changed.
...
### High — admin routes and URL secrets (fixed in source)

Every admin handler now performs route-level Bearer-or-global-admin authorization. URL secret/token support was removed; machine callers must use `Authorization: Bearer`. This requires scheduler coordination before deployment.
